...
__method__ = ['Non-Private', 'Original-DPSGD', 'Papernot et al.', 'Dynamic-Momentum (Ours)']
...
__color__ = {'Non-Private': '#023EFF', 'Original-DPSGD': '#FF7C00',
             'Papernot et al.': '#1AC938', 'Dynamic-Momentum (Ours)': '#E8000B'}
for i in range(4):
    data = {}
    data['method'] = []
    data['acc'] = []
    ...
    filelist = json.load(open('file3.json'))
    ...
    if i == 0:
        ax[0][i].set_ylim(0.8, 1.0)
    elif i == 1:
        ax[0][i].set_ylim(0.7, 0.95)
    elif i == 2:
        ax[1][i % 2].set_ylim(0.3, 0.8)
    elif i == 3:
        ax[1][i % 2].set_ylim(0.2, 0.8)
    ...
    if i == 2:
        ...
        ax[1][i % 2].legend(loc=2, bbox_to_anchor=(0.05, -0.1), borderaxespad=1, ncol=10, handlelength=5, columnspacing=20, frameon=False, markerscale=20,
                            handles=handles, labels=['Non-Private', 'Original-DPSGD', 'Papernot et al.', 'Dynamic-Momentum (Ours)'])
...
plt.savefig('dpsgd_compare_all_acc.pdf', bbox_inches='tight', pad_inches=0.2)
